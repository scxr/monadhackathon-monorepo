...
async function getPostsByUser(userAddress: string) {
    let start = Date.now();
    const response = await fetch(indexer_endpoint as string, {
        method: 'POST',
        headers: {
...
async function getPostsWithOffsetAndLimit(offset: number, limit: number) {
    let start = Date.now();
    const response = await fetch(indexer_endpoint as string, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ query: `{
            ChainSocial_PostCreated(offset: ${offset}, limit: ${limit}) {
                id
                content
                author
            }
